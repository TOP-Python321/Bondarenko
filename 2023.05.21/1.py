...
def pick_resistors(resist: int) -> dict[str, tuple[int, ...]] | None:
    if not 100 <= resist < 1000:
        return None

    result = {}
    for key, value in nominals.items():
        min_diff = min(map(lambda r: abs(r - resist), value))
        result[key] = tuple(filter(
            lambda r : abs(r - resist) == min_diff, value
        ))
    return result
